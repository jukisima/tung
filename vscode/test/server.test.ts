...
test("server implements the editor workflow over stdio", async (context) => {
  ...
  connection.listen();
  const request = (method, params) => connection.sendRequest<any>(method, params);
  ...
  connection.sendNotification("textDocument/didOpen", {
    textDocument: { uri: depUri, languageId: "tung", version: 1, text: depText },
  });
  assert.deepEqual((await cleanDepDiagnostics).diagnostics, []);
  const cleanDiagnostics = nextDiagnostics(connection, mainUri);
  connection.sendNotification("textDocument/didOpen", {
    textDocument: { uri: mainUri, languageId: "tung", version: 1, text: mainText },
  });
  ...
  connection.sendNotification("textDocument/didChange", {
    ...
    contentChanges: [
      { text: depText.replace("answer: integer = 42", "answer: string = 'changed'") },
    ],
  });
  ...
  assert.deepEqual(badResult.diagnostics[0].range.start, positionOf(badText, "value"));
  const missingSemicolon = "bring dep.tung";
  const parseDiagnostics = nextDiagnostics(
    connection,
    mainUri,
    ({ diagnostics }) =>
      diagnostics.length > 0 && /expected ';' after bring/.test(diagnostics[0].message),
  );
  connection.sendNotification("textDocument/didChange", {
...
test("server semantic tokens colour real shape methods", async (context) => {
  ...
  const file = path.join(root, "tongue", "bookhoard", "collection", "functor.tung");
  ...
  const semantic = await connection.sendRequest<any>("textDocument/semanticTokens/full", {
    textDocument: { uri },
  });
  const semanticAt = decodedSemanticTokens(
    semantic.data,
...
const nextDiagnostics = (connection, uri, accept: (params: any) => boolean = () => true) => {
