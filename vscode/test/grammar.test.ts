...
import { tokenModifiers } from "../server/semantic";
const extensionRoot = path.resolve(__dirname, "..", "..");
const grammar = JSON.parse(
  fs.readFileSync(path.join(extensionRoot, "syntaxes", "tung.tmLanguage.json"), "utf8"),
);
...
test("textmate gives a whole bring path one scope", async () => {
  ...
  for (const line of [
    ...
    const pathTokens = loaded
      .tokenizeLine(line)
      .tokens.filter((token) => token.startIndex < end && token.endIndex > start);
    assert.deepEqual(
...
test("textmate scopes block comments", async () => {
  const tokens = (await loadGrammar()).tokenizeLine("let x = 1; /* hidden { = */").tokens;
  const comment = tokens.find(({ scopes }) => scopes.includes("comment.block.tung"));
  assert(comment);
});
...
test("textmate recognises declaration and member keywords", async () => {
  const loaded = await loadGrammar();
  for (const [line, keyword] of [
    ...
    const token = loaded
      .tokenizeLine(line)
      .tokens.find(({ startIndex, endIndex }) => startIndex === 0 && endIndex === keyword.length);
    assert(token.scopes.includes("keyword.declaration.tung"), line);
  }
});
test("textmate recognises the law equation marker", async () => {
...
test("textmate keeps law expressions distinct from parameter types", async () => {
  ...
  assert(occurrences[1].scopes.includes("support.type.tung"), "the parameter annotation is a type");
  for (const occurrence of [occurrences[0], occurrences[2], occurrences[3]]) {
    ...
    assert.equal(occurrence.scopes.includes("support.type.tung"), false, "law terms are not types");
  }
});
test("textmate fallback keeps type and term categories distinct", async () => {
  ...
  const scopesOf = (name) => {
    ...
    return tokens.find(({ startIndex, endIndex }) => startIndex <= start && start < endIndex)
      .scopes;
  };
...
test("manifest maps semantic roles to theme scopes", () => {
  ...
  const semanticTypes = Object.fromEntries(
    manifest.contributes.semanticTokenTypes.map(({ id, superType }) => [id, superType]),
  );
  ...
  assert.deepEqual(scopes.shape, scopes.type, "shape names use the theme's type colour");
  assert.deepEqual(
    scopes.call,
    scopes.function,
...
test("semantic modifiers use standard lsp names or manifest contributions", () => {
  ...
  const contributed = new Set(manifest.contributes.semanticTokenModifiers.map(({ id }) => id));
  for (const modifier of tokenModifiers) {
    assert(standard.has(modifier) || contributed.has(modifier), modifier);
  }
});
test("textmate fallback does not treat bookhoard data types as primitives", () => {
