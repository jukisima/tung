...
the workspace index followeþ local and bundled imports, honoureþ `show`, understandeþ qualify-if-needed lookup, default last-segment import aliases, and explicit `bring path alias` overrides, and sendeþ unsaved imported sources to the haskell checker. the checker bridge askeþ cabal for `exe:tung` and falleþ back to a built executable under `tongue/dist-newstyle` when cabal lookup is unavailable. it keepeþ one versioned session alive for checking, inferred types, and formatting, and cancelleþ obsolete work after edits. an error-tolerant source model keepeþ navigation and highlighting available while code is incomplete; the haskell implementation remaineþ authoritative for diagnostics, inferred types, and formatting.
...
`make language-metadata` runneþ `tongue/tool/language-names.tung` and refresheþ the generated keyword and primitive-type table used by semantic highlighting.
