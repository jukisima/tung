// builds a self-contained standard-bookhoard wiki from public declarations.
import * as fs from "node:fs";
import * as path from "node:path";
import { WorkspaceIndex, toFilePath } from "./workspace";
const generateBookhoardHtml = (tongue = path.resolve(__dirname, "..", "..", "..", "tongue")) => {
  return renderPage(bookhoardModules(tongue));
};
const bookhoardModules = (tongue) => {
  const workspace = new WorkspaceIndex({ get: () => undefined, all: () => [] });
  const root = path.join(tongue, "bookhoard");
  ...
  return workspace
    ...
    .map((model) => ({
      ...
      definitions: workspace
        ...
        .filter((definition) => definition.uri === model.uri && !definition.local)
        .sort(
          (a, b) => roleOrder(a.role) - roleOrder(b.role) || a.bareName.localeCompare(b.bareName),
        ),
    }))
...
const renderPage = (modules) => {
  const count = modules.reduce((total, module) => total + module.definitions.length, 0);
  const navigation = modules
    .map(
      (module) =>
        `<a href="#${moduleId(module.name)}" data-module-link="${html(module.name.toLowerCase())}">${html(module.name)}</a>`,
    )
    .join("\n");
...
const renderModule = (module) => {
  const search = [module.name, ...module.definitions.flatMap(definitionSearch)]
    .join(" ")
    .toLowerCase();
  return `<section class="module" id="${moduleId(module.name)}" data-module="${html(module.name.toLowerCase())}" data-search="${html(search)}">
  <header class="module-head">
    <h2>${html(module.name)}</h2>
    <a href="${sourceHref(module.name)}">source</a>
  </header>
  ${module.definitions.map((definition, index) => renderDefinition(module.name, definition, index)).join("\n  ")}
</section>`;
};
const renderDefinition = (moduleName, definition, index) => {
  ...
  const detail = definition.detail
    ...
    : "";
  return `<article id="${id}" data-definition data-search="${html(definitionSearch(definition).join(" ").toLowerCase())}">
...
const roleOrder = (role) => {
  return { type: 0, shape: 1, enumMember: 2, method: 3, function: 4, variable: 5 }[role] ?? 9;
};
const moduleName = (bookhoard, model) => {
  return path.relative(bookhoard, toFilePath(model.uri)).replaceAll(path.sep, "/");
};
const moduleId = (name) => {
  return `module-${Buffer.from(name).toString("base64url")}`;
};
const definitionId = (moduleName, definition, index) => {
  return `name-${Buffer.from(`${moduleName}:${definition.role}:${definition.bareName}:${index}`).toString("base64url")}`;
};
const sourceHref = (name) => {
  return `../../tongue/bookhoard/${name.split("/").map(encodeURIComponent).join("/")}`;
};
const oneLine = (text) => {
...
if (require.main === module) {
  const tongue = path.resolve(process.argv[2] || path.join(__dirname, "..", "..", "..", "tongue"));
  const output = process.argv[3] && path.resolve(process.argv[3]);
  const docs = generateBookhoardHtml(tongue);
  if (output) {
