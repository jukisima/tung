...
parserTests =
  runGroup "parser" $
    ...
    , expectParseOk "grouped show-type declaration" "data left { left } data right { right } show-type left, right;"
    ...
    , expectParseOk "parameterized state effect" "effect a state { 𝟙 get: a, a set: 𝟙 }"
...
typeTests =
  runGroup
    ...
    , expectTypeOk "foreign division may fail" "class foreign { integer divide-integer integer: integer ! string fail } let ok: integer = try 4 divide-integer 2 { fail | 0 }"
    , expectTypeOkWithImports "show reexports value and type" "bring file.jbini; let ok: box = box" (Map.fromList [("file.jbini", "data box { box } show-type box; show box;")])
    ...
    , expectTypeOk "concrete flesh need is solved by existing flesh" "bone a add { a + a: a } bone a multiply { a × a: a } data natural { zero, natural suc } flesh natural add { let a + b = a } flesh natural multiply { let a × b = a + b }"
    ...
    , expectRunnableErr "runner rejects custom effect" "data 𝟙 { null } effect ask { 𝟙 ask: 𝟙 } let _ = null ask;"
    , expectEq "non-unit runnable result rejected" (checkRunnableWithImports "42" Map.empty) "type error:"
    , expectEq "unhandled fail rejected at runnable boundary" (checkRunnableWithImports "1 ÷ 0" Map.empty) "type error:"
    ]
...
evaluationTests = do
  ...
  runGroup
    ...
    , expectEval "foreign native call" "class foreign { integer add-integer integer: integer } 1 add-integer 2" "eval ok: 3"
    , expectEval "foreign native through flesh" "class foreign { integer add-integer integer: integer } bone a plus-host { a plus-host a: a } flesh integer plus-host { let plus-host = add-integer } 1 plus-host 2" "eval ok: 3"
    ...
    , expectEval "handled fail payload with return clause" "try 1 ÷ 0 { return n | n to-string, message fail | message }" "eval ok: 'division by zero'"
    , expectEval "handler return changes normal result" "try 2 + 3 { return n | n to-string }" "eval ok: '5'"
    , expectEval "partial effect only runs when full" "try 0 (1 ÷) { fail | 7 }" "eval ok: 7"
    , expectEval "operation handler resumes" "effect ask { integer ask: integer } try 10 ask { x ask | (x + 1) resume }" "eval ok: 11"
