module Jbini.Syntax
  ( Program(..), Decl(..), EffectOp(..), Ctor(..), BoneMember(..), BoneNeed(..)
  , TypeAnn(..), TypeExpr(..), Expr(..), HandlerCase(..), RecordUpdate(..), Pattern(..), MatchCase(..)
  , Token(..)
  , matchCaseArity, matchRows
  ) where
...
data Token
  = TIdent String | TInteger Int | TFloat String | TChar String | TString String
  | TLet | TGiven | TBring | TType | TData | TEffect | TBone | TFlesh | TMatch | TTry
  | TLParen | TRParen | TLBrace | TRBrace | TLBracket | TRBracket
  | TColon | TComma | TMapsTo | TArrow | TForall | TBang | TEquals | TSemicolon | TDot | TDollar
  deriving (Eq, Show)
