...
importedCases imports =
  ...
  , evalOkWith "imported fill doth not shadow native integer order" "use ilk/list.tung yield 0 till 2" imports "eval ok: (0 (1 (2 empty _*) _*) _*)"
  ...
  , evalOkWith "non-total powerset lattice joineþ predicates" "use ground.tung use ilk/list.tung use ilk/powerset.tung use collection/catamorphism.tung let one: integer powerset = { x @ x ≡ 1 } let two: integer powerset = { x @ x ≡ 2 } let joined = one _* (two _* list.empty) $ …∨ yield joined ∋ 2" imports "eval ok: yea"
  ...
  , evalOkWith "list indexing is zero based" "use ilk/list.tung yield (10 _* (20 _* empty)) at 1" imports "eval ok: (20 some)"
  , evalOkWith "negative list index is absent" "use ground.tung use ilk/list.tung let index = 0 - 1 yield (10 _* empty) at index" imports "eval ok: none"
  ...
  , evalOkWith "natural semiring supplieþ multiplicative monoid evidence" "use ground.tung use ilk/list.tung use ilk/natural.tung use collection/catamorphism.tung let two: natural = (natural.zero suc) suc let three = two suc yield ((two _* (three _* list.empty)) …×) to-integer" imports "eval ok: 6"
  ...
  , evalOkWith "powerset infimal wrapper useþ intersection" "use ground.tung use ilk/list.tung use ilk/powerset.tung use algebra/total/semigroup.tung use collection/catamorphism.tung let one: integer powerset = { value @ value ≡ 1 } let both: integer powerset = { value @ (value ≡ 1) ∨ (value ≡ 2) } let wrapped = (one infimal) _* ((both infimal) _* list.empty) yield match wrapped fold { result infimal @ result ∋ 2 }" imports "eval ok: nay"
