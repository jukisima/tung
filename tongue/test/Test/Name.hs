-- bare and qualified lookup, ambiguity, exports, shadowing, and record access.
...
resolutionCases =
 ...
 where
  ...
  privateType = Map.fromList [("private-type.tung", "let-ilk secret = ℤ")]
  reexported =
    Map.fromList
  ...
  selectiveReexports =
    Map.fromList
      [ ("type-only.tung", "let-ilk token = ℤ let token = 1 show-ilk token")
      , ("term-only.tung", "let-ilk token = ℤ let token = 1 show token")
      ]
  identityExport =
    Map.fromList
  ...
  aliasImports =
    Map.fromList
      [ ("integer-type.tung", "show let-ilk token = ℤ")
...
lookupCase = case contextOf "use left.tung" imports of
  ...
  Right ctx -> case lookupEnv "foo" ctx of
    EnvFound (Forall _ _ ty) -> expectEq "lookup imported value" "ℤ" (showTy ty)
