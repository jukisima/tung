...
accepted =
  ...
  , ("record update changeth a field type", "let person = r(value = 1) let changed: r(value: text) = r(= person, value = 'one')")
  , ("record removal changeth the row", "let person = r(name = 'n', age = 1) let public: r(name: text) = r(= person, - age)")
  ...
  , ("specific fill outranketh a blanket fill", "shape a identity { let a identity: a } fill a identity { let x identity = x } fill integer identity { let x identity = x } let ok = 1 identity")
...
rejected =
  ...
  , ("occurs check followeth nested records", "let x omega = r(value = x) x")
  ...
  , ("closed record rejecteth extra field", "let bad: r(name: text) = r(name = 'n', age = 1)")
  ...
  , ("record update needeth a record", "let bad = r(= 1, value = 2)")
