...
accepted =
  ...
  , ("type ascription preserveþ an immediate effect", unitData ++ "deed pulse { 𝟙 pulse: integer } let (_: 𝟙) run: integer ! pulse = (only pulse: integer)")
  ...
  , ("effect parameters shadow type aliases", "ilk 𝟙 { only } let-ilk a = integer deed a state { 𝟙 get: a } let good: 𝟙 → text ! text state = get")
  ...
  , ("alternative rows contribute to coverage", boolData ++ "let ok: integer = match yea, nay { yea, _ | _, yea @ 1, nay, nay @ 0 }")
  ...
  , ("text match with fallback", "let ok: integer = match 'yes' { 'yes' @ 1, _ @ 0 }")
  , ("text patterns in a function", "let classify: text → integer = { 'yes' @ 1, _ @ 0 }")
...
rejected =
  ...
  , ("effectful let result stayeþ monomorphic", unitData ++ "deed a state { 𝟙 get: a } let _ run = (let value = only get let number: integer = value let word: text = value yield only)")
...
importCases =
  ...
  , typeOkWith "unresolved raw host data dependencies remain ABI-compatible" "use ilk/table.tung standard ilk k table v { ((k ∏ v) list) from-list } let cast: integer table text → integer standard.table text = { x @ x }" rawTableImport
  ...
  , runnableErr "main takeþ one unit argument" (unitData ++ "let (_: integer) main: 𝟙 = only")
...
boundCases imports =
 ...
 where
  ...
  powersetPrefix = "use ground.tung use ilk/list.tung use ilk/powerset.tung use collection/catamorphism.tung let empty-set: integer powerset = powerset.empty let full-set: integer powerset = universe "
...
numericHierarchyCases imports =
  [ typeOkWith "integer supplieþ euclidean division" "use ground.tung use ilk/product.tung graiþ a euclidean let (x: a, y: a) divide-with-rest: a ∏ a ! text fail = x % y let value: integer ∏ integer = try -5 divide-with-rest 3 { fail @ 0 ∏ 0 }" imports
